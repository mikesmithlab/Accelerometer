...
void QMI8658_config_ae(enum QMI8658_AeOdr odr)
{
	...
	QMI8658_config_gyro(QMI8658GyrRange_2048dps, QMI8658GyrOdr_8000Hz, QMI8658Lpf_Disable,  QMI8658St_Disable);
	QMI8658_config_acc(QMI8658AccRange_8g, QMI8658AccOdr_8000Hz, QMI8658Lpf_Disable,  QMI8658St_Disable);
...
void QMI8658_read_acc_xyz(float acc_xyz[3], unsigned int *tim_count)
{ //modified inputs to include time pointer
	unsigned char buf_reg[6];
	//short raw_acc_xyz[3];

	QMI8658_read_reg(QMI8658Register_Ax_L, buf_reg, 6); // 0x19, 25
	acc_xyz[0] = (short)((unsigned short)(buf_reg[1] << 8) | (buf_reg[0]))* (ONE_G / acc_lsb_div);
	acc_xyz[1] = (short)((unsigned short)(buf_reg[3] << 8) | (buf_reg[2]))* (ONE_G / acc_lsb_div);
	acc_xyz[2] = (short)((unsigned short)(buf_reg[5] << 8) | (buf_reg[4]))* (ONE_G / acc_lsb_div);
